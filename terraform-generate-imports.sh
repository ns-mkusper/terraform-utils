...
echo "$LIST" | while IFS= read -r entry; do
    ...
    if [[ $entry == aws_acm_certificate_validation.* ]] ||
        [[ $entry == *.aws_acm_certificate_validation.* ]]; then
        echo "$total_items Skipping entry: $entry"
        echo "# $entry" >>"$OUTPUT_FILE"
        echo "# skip importing aws_acm_certificate_validation resource as it's not importable" >>"$OUTPUT_FILE"
        echo "" >>"$OUTPUT_FILE"
        continue
    fi
